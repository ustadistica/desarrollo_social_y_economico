...
code(
    ...
    "    print('ADVERTENCIA: variable NBI no disponible — revisar data/cruce_secop_dane_sprint2.parquet')\n"
    "else:\n"
...
# ─────────────────────────────────────────────────────────────────────────────
md(
    "## 6. Equidad territorial — Gini de inversión per cápita\n\n"
    "El Gini territorial se calcula exclusivamente sobre **inversión per cápita** "
    "(`inversion_total_monto / poblacion_censo_2018`). Esta es la métrica reportable "
    "para equidad porque compara el peso público por habitante y no el tamaño absoluto "
    "del municipio.\n\n"
    "Reglas metodológicas: se incluyen sólo municipios con `inversion_total_monto > 0` "
    "y `poblacion_censo_2018 > 0`; no se imputan ceros con `fillna(0)`. Se muestra una "
    "serie con Bogotá y otra sin Bogotá como sensibilidad al sesgo de imputación del "
    "SECOP al municipio de la entidad contratante."
)

code(
    "# ── Gini territorial de inversión per cápita ───────────────────────\n"
    "# Métrica oficial de equidad: monto contratado por habitante.\n"
    "# Reglas: se requiere monto > 0 y población > 0; no se imputan ceros.\n"
    "\n"
    ...
    "\n"
    "    sub_ipc = sub[(sub[COL_MONTO] > 0) & (sub[COL_POB] > 0)].copy()\n"
    "    sub_ipc['ipc'] = sub_ipc[COL_MONTO] / sub_ipc[COL_POB]\n"
    "    g_ipc = gini(sub_ipc['ipc'].values)\n"
    "\n"
    "    sub_ipc_nb = sub_ipc[sub_ipc[COL_DIVIPOLA] != COD_BOGOTA]\n"
    "    g_ipc_no_bog = gini(sub_ipc_nb['ipc'].values)\n"
    "\n"
    "    filas_gini.append({\n"
    "        'año': int(a),\n"
    "        'gini_inv_per_capita': g_ipc,\n"
    "        'gini_inv_per_capita_sin_Bogotá': g_ipc_no_bog,\n"
    "        'n_muni_ipc': len(sub_ipc),\n"
    "    })\n"
    "\n"
    "gini_df = pd.DataFrame(filas_gini).set_index('año')\n"
    "\n"
    "fig, ax = plt.subplots(figsize=(10, 5))\n"
    "x = np.arange(len(gini_df.index)); w = 0.38\n"
    "ax.bar(x - w/2, gini_df['gini_inv_per_capita'], w,\n"
    "       label='Con Bogotá', color='#2980B9', edgecolor='black', alpha=0.85)\n"
    "ax.bar(x + w/2, gini_df['gini_inv_per_capita_sin_Bogotá'], w,\n"
    "       label='Sin Bogotá', color='#5DADE2', edgecolor='black', alpha=0.85)\n"
    "ax.set_xticks(x); ax.set_xticklabels(gini_df.index)\n"
    "ax.set_ylim(0, 1.05)\n"
    "ax.set_title('Equidad territorial — Gini de inversión per cápita\\n'\n"
    "             '(métrica oficial de equidad)', fontweight='bold')\n"
    "ax.set_xlabel('Año'); ax.set_ylabel('Gini (monto / habitante)')\n"
    "ax.axhline(0.5, color='gray', ls=':', lw=1)\n"
    "ax.legend(fontsize=9, loc='lower left')\n"
    "for xi, (v1, v2) in enumerate(zip(gini_df['gini_inv_per_capita'],\n"
    "                                    gini_df['gini_inv_per_capita_sin_Bogotá'])):\n"
    "    ax.text(xi - w/2, v1 + 0.01, f'{v1:.2f}', ha='center', fontsize=7)\n"
    "    ax.text(xi + w/2, v2 + 0.01, f'{v2:.2f}', ha='center', fontsize=7)\n"
    "\n"
    "plt.tight_layout(); plt.show()\n"
    "\n"
    "# Diccionarios explícitos para el resumen ejecutivo\n"
    "gini_ipc = gini_df['gini_inv_per_capita'].to_dict()\n"
    "gini_ipc_sin_bogota = gini_df['gini_inv_per_capita_sin_Bogotá'].to_dict()\n"
    "\n"
    "print('\\nGini territorial por año — inversión per cápita:')\n"
    "display(gini_df.round(4).rename(columns={\n"
    "    'gini_inv_per_capita':            'Gini IPC',\n"
    "    'gini_inv_per_capita_sin_Bogotá': 'Gini IPC sin Bogotá',\n"
    "    'n_muni_ipc':                     'n municipios IPC',\n"
    "}))\n"
    "\n"
    "print('\\nLectura recomendada:')\n"
    "print('  - Métrica oficial: Gini IPC.')\n"
    "print('  - Gini IPC sin Bogotá es una sensibilidad por el sesgo de imputación del SECOP.')\n"
    "print('  - El cálculo no usa fillna(0); trabaja sólo con municipios con monto y población positivos.')\n"
)
...
code(
    ...
    "    print('      Fuente alternativa: data/silver/silver_emicron_agregado.parquet')\n"
)
...
code(
    ...
    "\n"
    "print('\\nEquidad territorial — Gini de inversión per cápita:')\n"
    "print('  (interpretar SIN Bogotá; con Bogotá hay sesgo por imputación del orden nacional)')\n"
    "for a, g_val in gini_ipc.items():\n"
    "    g_sb = gini_ipc_sin_bogota.get(a, np.nan)\n"
    "    if pd.isna(g_val):\n"
    "        print(f'  {a}: N/A')\n"
    "    else:\n"
    "        barra = '|' * int(g_sb * 30) if pd.notna(g_sb) else ''\n"
    "        print(f'  {a}: con Bogotá={g_val:.3f}  |  sin Bogotá={g_sb:.3f} [{barra}]')\n"
    "\n"
