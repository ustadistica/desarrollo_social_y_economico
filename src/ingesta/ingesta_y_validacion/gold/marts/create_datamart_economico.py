...
def _calcular_vocacion_productiva(fact_tejido_productivo_df: pd.DataFrame) -> pd.DataFrame:
    """
    Calcular vocación productiva por municipio.
    
    Parameters:
    - fact_tejido_productivo_df: Tabla de hechos de tejido productivo
    
    Returns:
    - DataFrame con vocación productiva por municipio
    """
    ...
    # Definir agregaciones dinámicas
    agg_dict = {}
    if 'codigo_ciiu' in fact_tejido_productivo_df.columns:
        agg_dict['codigo_ciiu'] = lambda x: x.mode().iloc[0] if len(x.mode()) > 0 else None
    
    # Mapear columnas de establecimientos si existen
    for col in ['economia_popular_unidades', 'total_micronegocios', 'establecimientos_industria', 'total_establecimientos']:
        if col in fact_tejido_productivo_df.columns:
            agg_dict[col] = 'sum'
            
    if 'tasa_formalizacion' in fact_tejido_productivo_df.columns:
        agg_dict['tasa_formalizacion'] = 'mean'
    
    if not agg_dict:
        # Fallback si no hay nada que agregar más que el índice
        return pd.DataFrame(columns=['divipola_municipio', 'sector_predominante_tp', 'economia_popular_share', 'formalizacion_rate'])

    vocacion = fact_tejido_productivo_df.groupby('divipola_municipio').agg(agg_dict).reset_index()
    
    # Renombrar y asegurar columnas esperadas
    vocacion = vocacion.rename(columns={
        'codigo_ciiu': 'sector_predominante_tp',
        'tasa_formalizacion': 'formalizacion_rate',
    })
    ...
    if 'total_micronegocios' not in vocacion.columns: vocacion['total_micronegocios'] = 1 # Evitar división por cero
    
    # Calcular share de economía popular
